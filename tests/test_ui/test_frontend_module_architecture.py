...
def test_single_dispatcher_owns_base_function_extension() -> None:
    live = _read("multi_live.js")
    settings = _read("settings_cleanup.js")
    history = _read("file_history.js")
    meeting = _read("meeting.js")

    assert "const ultraBaseUI =" in live
    assert "function ultraRegisterModule" in live
    assert "window.UltraUI = Object.freeze" in live
    ...
    for source in (settings, history, meeting):
